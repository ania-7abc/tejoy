...
AckModule::AckModule(std::shared_ptr<event_system::EventBus> bus, nlohmann::json &config)
	: Module(std::move(bus), config), work_guard_(boost::asio::make_work_guard(io_context_))
{
	...
	io_thread_ = std::jthread([this]() -> void { io_context_.run(); });
}
...
{
	subscribe<events::detail::SendRawUpdateRequest>(
		[this](const auto &event) -> auto { on_send_update_request(event); });
	subscribe<events::detail::UpdateReceived>([this](const auto &event) -> auto { on_update_received(event); });
	subscribe_update(UpdateTypes::ACK, [this](const auto &event) -> auto { on_ack_received(event); });
}
...
{
	std::scoped_lock lock(pending_mutex_);
	if (const auto found_update = pending_.find(event.data.at("pkg_id").get<uint32_t>());
		found_update != pending_.end())
	{
...
{
	if (event.no_ack || event.type == UpdateTypes::ACK)
		return;

	publish<events::detail::SendUpdateRequest>(nlohmann::json({{"pkg_id", event.pkg_id}}), UpdateTypes::ACK,
											   event.sender, true);
}
...
void AckModule::start_timer(PendingUpdate &update)
{
	...
	update.timer->async_wait([this, pkg_id = update.pkg_id](const auto &error_code) -> auto {
		if (!error_code)
