...
DiscoveryModule::DiscoveryModule(std::shared_ptr<event_system::EventBus> bus, nlohmann::json &config)
	: Module(std::move(bus), config), work_guard_(boost::asio::make_work_guard(io_context_)), timer_(io_context_)
{
	...
	publish<events::RequestI>([promise_i](const User &user) mutable -> void { promise_i->set_value(user); });
	i_ = future_i.get();
...
{
	subscribe<events::RequestDiscoveryIp>([this](auto &event) -> auto { event.on_result(discovery_ip_); });
	subscribe_update(UpdateTypes::DIS_FIND, [this](auto &event) -> auto { on_dis_find_received(event); });
	subscribe_update(UpdateTypes::DIS_OK, [this](auto &event) -> auto { on_dis_ok_received(event); });
}
...
void DiscoveryModule::start_timer()
{
	...
	timer_.async_wait([this](auto error_code) -> auto {
		if (!error_code)
			on_timer();
	});
}
