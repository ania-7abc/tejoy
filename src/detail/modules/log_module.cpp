...
LogModule::LogModule(std::shared_ptr<event_system::EventBus> bus, nlohmann::json &config)
	: Module(std::move(bus), config)
{
	...
	config_.emplace("reply", nlohmann::json::object());
	config_.at("reply").emplace("event", false).first.value().get_to(reply_event_);
	config_.at("reply").emplace("console", false).first.value().get_to(reply_console_);
	config_.at("reply").emplace("file", "").first.value().get_to(reply_file_);
...
{
	subscribe<event_system::AnyEvent>([this](auto &event) -> auto { on_any_event(event); });
}
