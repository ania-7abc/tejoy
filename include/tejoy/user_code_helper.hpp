...
class UserCodeHelper : public event_system::Subscriber
{
  public:
	...
	template <typename EventType> void on(const std::function<void(const EventType &event)> &handler)
	{
		handlers.push_back(
			bus()->make_subscriber<EventType>([handler = std::move(handler)](auto &event) -> auto { handler(event); }));
	}
	...
	void on_update_send_error(const std::function<void(const events::UpdateSendError &error)>& handler);
