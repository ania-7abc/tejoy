...
{
    AckUpdateReceived(uint32_t pkg_id, User from) : pkg_id(pkg_id), from(std::move(from))
    {
    }
    uint32_t pkg_id;
    User from;
...
{
    explicit AlloUpdateReceived(User from) : from(std::move(from))
    {
    }
    User from;
};
...
{
    explicit ImokUpdateReceived(User from) : from(std::move(from))
    {
    }
    User from;
};
...
struct SendConfiguredUpdateRequest : event_system::Event
{
    SendConfiguredUpdateRequest(nlohmann::json data, std::string type, User recipient, bool no_encrypt = false,
                                bool no_ack = false)
        : data(std::move(data)), type(std::move(type)), recipient(std::move(recipient)), no_encrypt(no_encrypt), no_ack(no_ack)
    {
    }
    nlohmann::json data;
    std::string type;
