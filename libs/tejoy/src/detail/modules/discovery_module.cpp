...
namespace tejoy::detail::modules
{
  ...
  DiscoveryModule::DiscoveryModule(event_system::EventBus &bus, nlohmann::json &config) : Module(bus, config), timer_(io_context_) {}
  ...
  void DiscoveryModule::on_start()
  {
    ...
    ping_interval_s_ = config_.emplace("ping_interval_s", (std::size_t)15).first.value().get<std::size_t>();
    anonymous_ = config_.emplace("anonymous", false).first.value().get<bool>();
    ...
    subscribe<events::RequestDiscoveryIp>([this](auto &e)
                                          { e.promise.set_value(discovery_ip_); });
    subscribe<events::RequestDiscoveryPort>([this](auto &e)
                                            { e.promise.set_value(discovery_port_); });

    subscribe<events::AlloUpdateReceived>([this](auto &e)
                                          { on_allo_received(e); });
    subscribe<events::ImokUpdateReceived>([this](auto &e)
                                          { on_imok_received(e); });
  ...
  void DiscoveryModule::on_imok_received(const events::ImokUpdateReceived &e)
  {
    publish<events::DiscoveredNewNode>(e.from);
  }
  ...
  {
    User to{.ip = discovery_ip_, .port = discovery_port_};
    publish<events::SendConfiguredUpdateRequest>(nlohmann::json::object(), "allo", to, true, true);
    start_timer();
  }
